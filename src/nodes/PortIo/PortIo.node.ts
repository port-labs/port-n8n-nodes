...
async function getAccessToken(
  ...
  try {
    ...
    if (!accessToken) {
      throw new Error(`Failed to obtain access token. Response: ${JSON.stringify(response)}`);
    }
...
export class PortIo implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'Port.io',
    name: 'portIo',
    icon: 'fa:plug', // TODO: Replace with actual icon
    group: ['transform'],
    version: 1,
    description: 'Invoke Port AI agents, call general AI interactions, and fetch invocation results',
    ...
    properties: [
      {
        ...
        type: 'options',
        options: [
          { name: 'Invoke a specific agent', value: 'invokeAgent', description: 'POST /v1/agent/:agentIdentifier/invoke' },
          { name: 'General-purpose AI interactions', value: 'generalInvoke', description: 'POST /v1/ai/invoke' },
          { name: "Get an invocation's result", value: 'getInvocation', description: 'GET /v1/ai/invoke/:invocation_identifier' },
        ],
        default: 'invokeAgent',
      },
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    const headerPairs = (this.getNodeParameter('additionalHeaders.headers', 0, []) as Array<{ name: string; value: string }>);
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (operation === 'invokeAgent') {
        ...
        const rawResponse = await this.helpers.httpRequest({
          ...
          headers: { Authorization: `Bearer ${accessToken}`, Accept: 'application/json', 'Content-Type': 'application/json', ...extraHeaders },
          body: payload,
      ...
      if (operation === 'generalInvoke') {
        ...
        try {
          ...
          throw new Error('Tools must be a valid JSON array (e.g., ["tool1", "tool2"])');
        }
